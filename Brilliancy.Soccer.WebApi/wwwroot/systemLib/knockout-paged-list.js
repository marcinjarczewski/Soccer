...
var PagedList = (function ($, ko) {
    ...
    function PagedList(option) {
        ...
        function BuildQueryOptions() {
            ...
            var filter = ko.mapping.toJS(self.filter());
            for (var propertyName in filter) {
                if (filter[propertyName])
                {
                    if (filter[propertyName].getMonth)
                    {
                        filter[propertyName] = moment(filter[propertyName]).format('YYYY-MM-DD');
                    }
                }
            }
            // Filtering options
            $.extend(queryOptions, filter);
